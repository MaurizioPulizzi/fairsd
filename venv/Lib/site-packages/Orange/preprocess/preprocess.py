...
class Discretize(Preprocess):
    ...
    def __call__(self, data):
        ...
        def transform(var, cut_points=False):       #cut_points aggiunto da maurizio
            if cut_points:                          #maurizio
                return method(data, var, True)      #maurizio
        ...
        method = self.method or discretize.EqualFreq()
                                                                    # maurizio
        return transform(data.domain.attributes[0], cut_points=True) #maurizio
